...
void setup(void)
{
  Timer1.initialize(5);  // 40 us = 25 kHz
  Serial.begin(9600);
  digitalWrite(2,HIGH);
}
...
//  for (float dutyCycle = 0; dutyCycle < 100.0; dutyCycle++) {
...
//    delay(500);
//  }
//for (float dutyCycle = 0; dutyCycle < 1024; dutyCycle++) {
//    Serial.print("PWM Fan, Duty Cycle = ");
//    Serial.println(dutyCycle);
...
if (Serial.available() > 0) {
  dutyCycle=Serial.readString();
  dutyCycle1= dutyCycle.toInt();
  Timer1.pwm(9, dutyCycle1);
  Serial.print("PWM Fan, Duty Cycle = ");
